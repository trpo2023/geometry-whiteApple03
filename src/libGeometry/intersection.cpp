...
bool Intersection::polygon_with_polygon(
        std::vector<std::vector<double>> first_coord, std::vector<std::vector<double>> second_coord)
{
    ...
    for (unsigned int i = 0; i < first_coord.size() - 1; i++) {
        ...
        for (unsigned int k = 0; k < second_coord.size() - 1; k++) {
            double x3 = second_coord[k][0], y3 = second_coord[k][1], x4 = second_coord[k + 1][0],
                   y4 = second_coord[k + 1][1];
...
bool Intersection::polygon_with_circle(

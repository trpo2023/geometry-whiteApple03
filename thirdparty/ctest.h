...
#define CTEST_DATA(sname)                           \
    template <typename T>                           \
    void CTEST_IMPL_SETUP_FNAME(sname)(T * data)    \
    {                                               \
    }                                               \
    template <typename T>                           \
    void CTEST_IMPL_TEARDOWN_FNAME(sname)(T * data) \
    {                                               \
    }                                               \
...
#define CTEST_IMPL_CTEST2(sname, tname, tskip)                                                    \
    ...
    static void (*CTEST_IMPL_SETUP_TPNAME(sname, tname))(struct CTEST_IMPL_DATA_SNAME(sname)*)    \
            = &CTEST_IMPL_SETUP_FNAME(sname)<struct CTEST_IMPL_DATA_SNAME(sname)>;                \
    static void (*CTEST_IMPL_TEARDOWN_TPNAME(sname, tname))(struct CTEST_IMPL_DATA_SNAME(sname)*) \
            = &CTEST_IMPL_TEARDOWN_FNAME(sname)<struct CTEST_IMPL_DATA_SNAME(sname)>;             \
    CTEST_IMPL_STRUCT(                                                                            \
            sname,                                                                                \
            tname,                                                                                \
            tskip,                                                                                \
...
void assert_wstr(
        const char* cmp, const wchar_t* exp, const wchar_t* real, const char* caller, int line);
...
void assert_dbl_compare(
        const char* cmp, double exp, double real, double tol, const char* caller, int line);
...
void assert_compare_u(const char* cmp, uintmax_t exp, uintmax_t real, const char* caller, int line)
{
    ...
    if (!get_compare_result(cmp, c3, c3 == 0)) {
        CTEST_ERR(
                "%s:%d  assertion failed, %" PRIuMAX " %s %" PRIuMAX, caller, line, exp, cmp, real);
    }
}
